...
def search_subject(keyword):
    cursor = conn.cursor()
    query = "SELECT * FROM second_subjects WHERE subject_name LIKE %s"
...
def main():
    ...
    time =""
    cursor = conn.cursor()
    cursor.execute(f"SELECT * FROM second_subjects WHERE lecture_time LIKE '%{day}%' AND lecture_time LIKE '%{time}%'")
    if cursor.fetchall():
